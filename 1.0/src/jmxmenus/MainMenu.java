		...
		while (choice == false) {
			...
			for (int i = 0; i < modules.size(); i++) {
				try {
					...
					// Process only class that are attack module
					// i.e. inheriting from "Attack" based class
					if (!"jmxattacks.Attack".equalsIgnoreCase(cl.getSuperclass().getName())) {
						continue;
					}
